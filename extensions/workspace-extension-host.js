...
(() => {
    ...
    function showToast(message, icon = "✓") {
        ...
        let notice = document.getElementById("jsyroExtensionNotice");
        if (!notice) {
            notice = document.createElement("div");
            notice.id = "jsyroExtensionNotice";
            notice.className = "jsyro-extension-notice";
            notice.setAttribute("role", "status");
            document.body.appendChild(notice);
        }

        notice.textContent = `${icon !== "✓" ? icon + " " : ""}${message}`;
...
.jsyro-extension-notice{position:fixed;top:82px;left:50%;z-index:2147483200;transform:translate(-50%,-10px);opacity:0;pointer-events:none;min-width:min(420px,calc(100vw - 32px));max-width:calc(100vw - 32px);padding:11px 18px;border:1px solid rgba(99,91,255,.55);border-radius:10px;background:rgba(21,24,33,.97);color:#edf0f7;box-shadow:0 14px 40px rgba(0,0,0,.42);backdrop-filter:blur(10px);text-align:center;font-size:12px;font-weight:700;line-height:1.4;transition:opacity .18s ease,transform .18s ease}
.jsyro-extension-notice.show{opacity:1;transform:translate(-50%,0)}
